...
```{r}
 proven17%>%
  dplyr::select(contains('A qué cree que se debe la delincuencia en el país: ¿Otra causa?')) %>%
  plot(breaks = 'jenks')
```
...
```{r}
proven17 %>% st_drop_geometry() %>%
  dplyr::select(TOPONIMIA, contains('A qué cree que se debe la delincuencia en el país: ¿Otra causa?'))
```
...
```{r}
proven17 %>%
  dplyr::select(contains('A qué cree que se debe la delincuencia en el país: ¿Otra causa?'), TOPONIMIA) %>%
  gather(variable, valor, -geom, -TOPONIMIA) %>% 
  group_by(TOPONIMIA) %>% 
  mutate(pct=round(valor/sum(valor)*100,2)) %>% dplyr::select(-valor) %>% 
  filter(variable %in% 'A qué cree que se debe la delincuencia en el país: ¿Otra causa?: No') %>% 
  ggplot() + aes(fill = pct) + geom_sf(lwd = 0.2) +
  facet_wrap(~variable) + theme(text = element_text(size = 10)) +
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(9, name = 'Reds')) +
  geom_sf_text(aes(label=pct), check_overlap = T, size = 3)
...
```{r}
proven17 %>% st_drop_geometry() %>% filter(ENLACE=='0314') %>% 
  dplyr::select(contains('A qué cree que se debe la delincuencia en el país: ¿Otra causa?'))
```
